...
import { NewOrderTimePage } from '../newordertime/newordertime';
...
export class AddorderPage {
  ...
  constructor(public navCtrl: NavController,
    ...
    public viewCtrl: ViewController,public menuCtrl: MenuController, private formBuilder: FormBuilder,
    ...
    platform.ready().then(() => {
      ...
      this.events.subscribe('menuopen',(is_open)=>{
          if(is_open){
  ...
  loadMap() {
    ...
    this.map.one(plugin.google.maps.event.MAP_READY, () => {
      ...
      //this.theMap.nativeElement.style.height = '100%';
  ...
  orderTime() {
    this.platform.ready().then(() => {
      //this.viewCtrl.dismiss();
      this.navCtrl.push(NewOrderTimePage, { "order": this.order });
      this.events.subscribe('order:updated:time', (data) => {
        let order = data.order;
        if (data.status == 'backbtn') {
          this.order = order;
          cordova.fireDocumentEvent('plugin_touch', {});
          //google.maps.event.trigger(this.map.loadMap(), 'resize');
          //this.map.refreshLayout();
          //this.map.setCameraTilt(this.map.getCameraTilt());
  ...
  async createMarker(location, label,hidden=false): Promise<{}> {
    return new Promise((resolve, reject) => {
      let icon;
      let me = this;

      this.map.animateCamera({ target: new LatLng(location.lat(), location.lng()), zoom: 13, duration: 1000 });
      var drag = true;
      if (label == "Pick Up") {
        // icon = {
        //   url:'/assets/img/pin_u.png'
        // }
        ...
        geocoder.geocode({
          ...
          if (status === google.maps.GeocoderStatus.OK) {
            ...
            if (!this.originMarker) {
              ...
              setTimeout(() => { this.map.animateCamera({ target: new LatLng(latlng.lat(), latlng.lng()), zoom: 13, duration: 700 }); }, 0);
              setTimeout(() => {
                if (drawRoude) {
                  this.drawRoude("Pick Up");
                }
  ...
  setPickUpCenterLocation(callback) {
    ...
    geocoder.geocode({
      ...
      if (status === google.maps.GeocoderStatus.OK) {
        ...
        if (!this.originMarker) {
          this.createMarker(latlng, "Pick Up",true).then((res) => {
            this.originMarker.setVisible(false);

          }, (err) => { });
        } else {
          this.originMarker.setPosition(new LatLng(latlng.lat(), latlng.lng()));
          //setTimeout(() => {this.map.animateCamera({ target: new LatLng(latlng.lat(), latlng.lng()), zoom: 13,duration:500  });}, 0);
        }
        setTimeout(() => {
          var address = "";
          for (var i = 0; i < results.length - 1; i++) {
      ...
      geocoder.geocode({
        ...
        if (status === google.maps.GeocoderStatus.OK) {
          if (!this.destMarker) {
            ...
            setTimeout(() => { this.map.animateCamera({ target: latlng, zoom: 13, duration: 700 }); }, 0);
            setTimeout(() => {
              if (drawRoude) {
                this.drawRoude("Drop Off");
              }
    ...
    geocoder.geocode({
      ...
      if (status === google.maps.GeocoderStatus.OK) {
        if (!this.destMarker) {
          ...
          this.createMarker(latlng, "Drop Off",true).then((res) => {
            this.destMarker.setVisible(false);
          }, (err) => { });
        } else {
          this.destMarker.setPosition(new LatLng(latlng.lat(), latlng.lng()));
          //  setTimeout(() => { this.map.animateCamera({ target: latlng, zoom: 13,duration:500  });}, 0);
        }
        setTimeout(() => {
          var address = "";
          for (var i = 0; i < results.length - 1; i++) {
        ...
        if (results[0]) {
          ...
          if (input === 'ORIG') {
            if (!me.originMarker) {
              ...
              me.map.animateCamera({ target: new LatLng(latlng.lat(), latlng.lng()), zoom: 13, duration: 700 });
